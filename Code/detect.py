...
# for webcam save video
def detect(save_img=True):
    imgsz = (320, 192) if ONNX_EXPORT else opt.img_size  # (320, 192) or (416, 256) or (608, 352) for (height, width)
    out, source, weights, half, view_img, save_txt = opt.output, opt.source, opt.weights, opt.half, opt.view_img, opt.save_txt
    webcam = source.isnumeric() or source.startswith('rtsp') or source.startswith('http') or source.endswith('.txt') or source.startswith('rtmp')

    print("start")
    print(source.startswith('rtmp'))
    # Initialize
    ...
    if webcam:
        ...
        #############################################
        # if LoadStreams, go line 120 to set p, s, im0 = path[i], '%g: ' % i, im0s[i].copy()
        #dataset = LoadStreams(source, img_size=imgsz)
        
        #############################################
        # if LoadWebcam, go line 121 to set p, s, im0 = path[i], '%g: ' % i, im0s.copy()
