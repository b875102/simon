...
class LoadVideos:  # for inference
            ...
            if len(files) == 0:
                ...
                for item in folder_content:
                    ...
                    if os.path.isdir(path + '/' + item):
                        in_files = sorted(glob.glob(os.path.join(path + '/' + item, '*.*')))
                        files += in_files
    ...
    def __next__(self):
        if self.count == self.nF:
            print('self.count stop: ',self.count)
            raise StopIteration
            ...
            while self.cap.isOpened():
                ...
                if not ret_val and self.frame >= self.nframes:
                    ...
                    if self.count == self.nF:  # last video
                        ...
                        ret_val, img0 = self.cap.read()
                if not ret_val:
                    print('cap.read fail skip : ',self.frame)
                    self.frame += 1
                if ret_val :
                    break
...
class LoadTinyVideos:  # for inference
    def __init__(self, path, img_size=416):
        path = str(Path(path))  # os-agnostic
        files = []
        if os.path.isdir(path):
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=416):
        self.path = str(Path(path))  # os-agnostic
        files = []
        if os.path.isdir(path):
            files = sorted(glob.glob(os.path.join(path, '*.*')))
    ...
    def __next__(self):
        ...
        path = self.path + '/' + str(self.count) + '.png'
        img0 = cv2.imread(path)  # BGR
